...
const Filter = ({ handleFindName, personsToFind }) => {
  return (
    <>
      filter shown with{" "}
      <input onChange={handleFindName} value={personsToFind} />
    </>
  );
};
  ...
  return (
    <>
      <form onSubmit={addName}>
        ...
        <div>
          number:{" "}
          <input onChange={handleChangeNumber} value={number} required />
        </div>
        <div>
    ...
    if (window.confirm("Delete " + personToEliminated.name + "?")) {
      deletePerson(personToEliminated)
        .then((status) => {
          notificationStateFromChild({
            status,
            message: personToEliminated.name + " eliminated.",
          });
          e.target.parentNode.parentNode.removeChild(e.target.parentNode);
          setTimeout(() => {
            notificationStateFromChild(null);
          }, 5000);
          let arrayPersons = persons.filter(
            (person) => person.id !== personToEliminated.id
          );
        ...
        .catch((error) => {
          notificationStateFromChild({
            status: error.status,
            message: `Information of ${personToEliminated.name} has already been removed from server.`,
          });
          setTimeout(() => {
            notificationStateFromChild(null);
          }, 5000);
    ...
    if (
      ...
      if (personExists === undefined) {
        ...
        if (
          ...
          updatePerson(personExists, { name: newName, number })
            .then((personUpdated) => {
              setNotification({
                status: 200,
                message: `${personUpdated.name}'s number was successfully updated.`,
              });
              setTimeout(() => {
                setNotification(null);
              }, 5000);
  ...
  return (
    <div>
      ...
      <Persons
        ...
        notificationStateFromChild={notificationStateFromChild}
      />
    </div>
  );
};
